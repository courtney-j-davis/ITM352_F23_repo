...
app.use(session({secret: "MySecretKey", resave: true, saveUninitialized: true}));
    ...
    //Answer to question: its the session id that came from the session id and the session came from the server.

app.get('set_cookie',  (req, res) =>{
    res.cookie('username', 'Courtney', {maxAge: 10000});
...
app.get('/use/session', (req,re) => {
    res.send(`Welcome, your session ID aas ${req.session.id}`)
    //req.session.destroy();
    ...
    let login_form = `
        <script>
            ...
            window.onload = function() {
                if (params.has('error')) {
                    login_form['username].value = params.get
                    ('username');
...
app.post("/login", function (request, response) {
    ...
    if (typeof user_reg_data[username_entered] != 'undefined') {
        ...
        if (password_entered == user_reg_data[username_entered].password) {
            ...
            console.log('Logiin cookie has been sent.')

            const userSession = request.session;
            if (!userSession.last.Login){
